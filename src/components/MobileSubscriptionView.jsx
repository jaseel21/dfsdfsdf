...
const MobileSubscriptionView = () => {
  const router = useRouter();
  const { startLoading, stopLoading } = useLoading();
  const [selectedAmount, setSelectedAmount] = useState(null);
  const [customAmount, setCustomAmount] = useState('');
  const [showSubscriptionForm, setShowSubscriptionForm] = useState(false);
  const [subscriptionType, setSubscriptionType] = useState('auto');
  ...
  // Filter periods based on subscription type
  const filteredPeriods = subscriptionType === 'auto' 
    ? periods.filter(period => period.value !== 'daily') 
    : periods;
  ...
  const validateForm = () => {
    ...
    if (!selectedAmount && !customAmount) return 'Please select or enter a subscription amount';
    if (customAmount && (parseInt(customAmount) <= 0 || isNaN(parseInt(customAmount)))) {
      return 'Please enter a valid subscription amount';
    }
    return null;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const subscriptionAmount = selectedAmount || parseInt(customAmount);
      ...
      if (!planResponse.ok) throw new Error(planData.error || 'Plan creation failed');
      ...
      if (subscriptionType === 'auto') {
        ...
        if (!subscriptionResponse.ok) throw new Error(subscriptionData.error || 'Subscription creation failed');
        ...
        return new Promise((resolve, reject) => {
          const options = {
            ...
            handler: async (response) => {
              try {
                // Direct success navigation without database save
                router.push(
                  `/subscription/success?amount=${subscriptionAmount}&name=${encodeURIComponent(
                    formData.fullName
                  )}&phone=${formData.phoneNumber}&period=${subscriptionPeriod}&method=${subscriptionType}&district=${
                    locationType === 'kerala' ? selectedDistrict : formData.district || 'Other'
                  }&panchayat=${locationType === 'kerala' ? formData.panchayat : ''}&paymentId=${response.razorpay_payment_id}&subscriptionId=${
            ...
            prefill: { name: formData.fullName, contact: formData.phoneNumber, email: formData.email },
            theme: { color: '#10B981' },
          };
        ...
        // Manual subscription - just create order for first payment
    ...
    <div className="relative flex flex-col min-h-screen bg-white overflow-hidden">
      ...
      <div className="relative z-10 p-6 pb-16 rounded-b-[50px] overflow-hidden">
        ...
        <div className="mt-2 items-left ml-5">
          ...
          <p className="text-white text-2xl font-bold mb-8 -mt-1.5">SUBSCRIPTION</p>
        </div>
      </div>
        ...
        <div className="flex justify-center items-center mt-6 mb-4 px-4">
          <div className="bg-[#AAA5FC] flex flex-row items-center justify-between w-full h-auto py-3 p-4 rounded-[10px]">
            <div className="flex items-end flex-col w-full">
              <p className="font-light text-[#1E1B4B] text-base">RECURRING</p>
              <p className="font-bold text-[#1E1B4B] text-base -mt-2">SUPPORT</p>
            </div>
            <div className="flex items-center justify-center w-40 mx-2">
              <span>
                <img src="/FontOF.svg" className="w-16" alt="Subscription" />
              </span>
            </div>
            <div className="flex items-start w-full">
              <p className="font-light text-[#1E1B4B] text-xs text-left">
                CONSISTENT<br />MONTHLY<br />SUPPORT
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-row space-x-1 mb-2 gap-1 mx-auto">
            {amounts.map((amount) => (
              <button
                ...
                className={`px-2 py-1 rounded-[10px] flex items-center justify-center border border-[#AAA5FC] h-[37px] text-sm ${
                  selectedAmount === amount && showSubscriptionForm ? 'bg-[#EFEEFF]' : 'bg-white'
                }`}
                style={{
                  background: selectedAmount === amount && showSubscriptionForm ? '#EFEEFF' : 'white',
                  boxShadow: '0px 1px 3px rgba(0, 0, 0, 0.1)',
                }}
                disabled={isSubmitting}
              >
                <span className="text-[#4A4A4A] text-[16px] font-medium">₹{amount}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex justify-center mt-2">
          <div>
            ...
            <button
              ...
              disabled={isSubmitting}
            >
              <span className="text-white font-medium text-lg text-center">Subscribe Now</span>
            </button>
          </div>
        </div>
      ...
      {showSubscriptionForm && (
        <>
          <div
            className="subscription-form-backdrop fixed inset-0 bg-black bg-opacity-50 z-40"
            onClick={toggleSubscriptionForm}
          />

          <div
            className="subscription-form-sheet fixed bottom-0 left-0 right-0 bg-gray-50 rounded-t-3xl shadow-lg z-50 h-[85%] overflow-y-auto"
            style={{
              transform: showSubscriptionForm ? 'translateY(0)' : 'translateY(100%)',
            }}
          >
            <div className="p-4 pb-8">
              ...
              <div className="text-center mb-6">
                <h3 className="text-[#3730A3] font-bold text-xl">Subscription Form</h3>
                <p className="text-gray-500 text-sm">Set up your recurring donation</p>
              </div>
              ...
              <div className="mb-6 px-4">
                <label className="block text-indigo-800 font-medium mb-3">Subscription Type</label>
              ...
              {/* Subscription Period */}
              <div className="mb-6 px-4">
                <label className="block text-indigo-800 font-medium mb-3">Payment Frequency</label>
                <div className="grid grid-cols-2 gap-2">
                  {filteredPeriods.map((period) => (
                    <button
                      key={period.value}
                      onClick={() => setSubscriptionPeriod(period.value)}
                      className={`py-2 px-2 rounded-xl text-center text-sm font-semibold ${
                  ...
                  {(selectedAmount || customAmount) && (
                    <div className="mt-4 bg-indigo-50 p-3 rounded-xl flex justify-between items-center">
                      <span className="text-indigo-700 font-medium">
                        {subscriptionType === 'auto' ? 'Auto' : 'Manual'} Subscription ({subscriptionPeriod}):
                      </span>
...
export default MobileSubscriptionView;